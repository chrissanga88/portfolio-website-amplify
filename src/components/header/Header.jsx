...
function Header({isWhite = false}) {
  ...
  /* useEffect is used because react components should not make network calls while rendering and they should not call async functions inside the component body. fetchAuthSession() is a network/async side-effect, so React requires it to be done inside useEffect. Amplify Auth session is not available synchronously, thus React cannot know if the user is logged in at the moment the component is rendered. useEffect runs only once when the header component loads, which prevents infinite loops and unnecessary checks. The isLoggedIn state allows for the component to initially render with no login status by using null as the default. As soon as the amplify auth session status is retrieved by the useEffect function, the UI is updated correctly to show whether or not the user is logged in */
  useEffect(() => {
